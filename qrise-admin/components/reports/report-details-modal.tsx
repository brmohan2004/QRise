...
import { AlertTriangle, Bug, ExternalLink, User, Calendar, ShieldAlert } from 'lucide-react'
          ...
          <div className="flex items-center gap-2 mb-2">
            {type === 'abuse' ? (
              ...
              <Badge variant="outline" className="bg-blue-500/10 text-blue-500 border-blue-500/20">
                <Bug className="h-3 w-3 mr-1" /> Bug Report
              </Badge>
            )}
            <Badge variant="outline" className="bg-gray-500/10 text-gray-500 border-gray-500/20 uppercase text-[10px]">
              {report.status}
            </Badge>
          </div>
          <DialogTitle className="text-xl font-bold">
            {type === 'abuse' ? `Abuse: ${report.reason}` : 'Bug Report Details'}
          </DialogTitle>
        ...
        <div className="space-y-6 mt-4">
            ...
            {type === 'abuse' ? (
              ...
              <>
                <div className="grid grid-cols-2 gap-4 mb-4">
                  <div>
                    <h4 className="text-xs font-bold text-gray-500 uppercase mb-2">Severity</h4>
                  ...
                  <div>
                    <h4 className="text-xs font-bold text-gray-500 uppercase mb-2">URL Affected</h4>
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-300 bg-[#0a0a0a] p-4 rounded-xl border border-[#1a1a1a] whitespace-pre-wrap">
                    {report.description}
                  </p>
                </div>
              </>
          ...
          {(report.action_taken || report.resolution_notes) && (
            <div className="bg-green-500/5 p-6 rounded-2xl border border-green-500/10">
              <h4 className="text-xs font-bold text-green-500/80 uppercase mb-2">
                {type === 'abuse' ? 'Action Taken' : 'Resolution Notes'}
              </h4>
              <p className="text-sm text-gray-300">
                {type === 'abuse' ? report.action_taken : report.resolution_notes}
              </p>
            </div>
          )}
        </div>
